...
const cprPattern = /\b\d{6}[-\s]?\d{4}\b/;
...
function analyzeText(text) {
    if (typeof text !== "string") {
        console.error("Fejl: analyzeText kræver en string som input.");
        return [];
    }
...
function fetchRiskFromModel(prompt) {
    if (typeof prompt !== "string") {
        console.error("Fejl: fetchRiskFromModel kræver en string som input.");
        return { matchPrompt: null, score: 0, label: 'Ukendt', source: 'Ingen match' };
    }
